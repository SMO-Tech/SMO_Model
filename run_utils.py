...
def get_ball_detections(
    ...
    # Filter by confidence - lower threshold for default COCO model
    ball_df = ball_df[ball_df["confidence"] > 0.25]
    
    # If using default COCO model, filter for "sports ball" class (class 37 in COCO)
    # Check if "ball" class exists, if not, use "sports ball"
    if len(ball_df) > 0 and "name" in ball_df.columns:
        # Try to filter for "ball" first (custom model), then "sports ball" (COCO)
        if "ball" in ball_df["name"].values:
            ball_df = ball_df[ball_df["name"] == "ball"]
        elif "sports ball" in ball_df["name"].values:
